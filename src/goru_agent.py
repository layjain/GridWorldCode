...
from src.networks.goru_net import GORU_NETWORK
import numpy as np
from tqdm import tqdm

class GORUAgent(BaseAgent):

    def __init__(self, config):
        super(GORUAgent, self).__init__(config)
        self.replay_memory = DRQNReplayMemory(config)
        self.net = GORU_NETWORK(4, config)
        self.net.build()
    ...
    def play(self, episodes, net_path, verbose=False):
        ...
        while i < episodes:
            ...
            if episode_steps==0 and verbose==True:
