...
from src.utils import conv2d_layer, fully_connected_layer, stateful_eunn, huber_loss
from src.networks.base import BaseModel


class EUNN_NETWORK(BaseModel):

    def __init__(self, n_actions, config):
        super(EUNN_NETWORK, self).__init__(config, "eunn")
    ...
    def add_logits_op_train(self):
        x=self.state
        out, state = stateful_eunn(x=x, num_layers=self.num_lstm_layers, lstm_size=self.lstm_size, state_input=tuple([self.lstm_state_train]),
    ...
    def add_logits_op_target(self):
        ...
        out, state = stateful_eunn(x, self.num_lstm_layers, self.lstm_size,
