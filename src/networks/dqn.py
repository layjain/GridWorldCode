...
class DQN(BaseModel):
    ...
    def train_on_batch_all_tf(self, state, action, reward, state_, terminal, steps):
        ...
        state_= state_
        _, q, train_loss, q_summary = self.sess.run(
            [self.train_op, self.q_out, self.loss, self.avg_q_summary], feed_dict={
                self.state: state,
                self.action: action,
        ...
        if self.train_steps % 1000 == 0:
            self.file_writer.add_summary(q_summary, self.train_steps)
            #self.file_writer.add_summary(colour_summary, self.train_steps)
        if steps % 20000 == 0 and steps > 50000:
            self.learning_rate *= self.lr_decay  # decay learning rate
            if self.learning_rate < self.learning_rate_minimum:
                self.learning_rate = self.learning_rate_minimum
    ...
    def add_placeholders(self):
        self.w = {}
        self.w_target = {}
        self.state = tf.placeholder(dtype = tf.float32, shape=[None, self.history_len, self.num_colors],
                                    name="input_state")
        ...
        self.state_target = tf.placeholder(tf.float32,
                                           shape=[None, self.history_len, self.num_colors],
                                           name="input_target")
    ...
    def add_logits_op_target(self):
        ...
        w, b, out = fully_connected_layer(x_flat, 512, scope_name="fully1_target")
    ...
    def add_loss_op_target(self):
        ...
        # self.merged_colour_sum = tf.summary.merge(self.colour_summary, "images")
