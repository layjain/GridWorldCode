...
class DRQN(BaseModel):
    ...
    def add_logits_op_train(self):
        x=self.state
        out, state = stateful_lstm(x=x, num_layers=self.num_lstm_layers, lstm_size=self.lstm_size, state_input=tuple([self.lstm_state_train]),
    ...
    def add_logits_op_target(self):
        ...
        out, state = stateful_lstm(x, self.num_lstm_layers, self.lstm_size,
